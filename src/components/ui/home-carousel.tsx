...
const SimpleCarousel: React.FC<{ activeIndex: number; onIndexChange: (idx: number) => void }> = ({ activeIndex, onIndexChange }) => {
  ...
  return (
    <div className="relative w-full w-full min-h-[900px] flex flex-col items-center justify-left">
 


      <div className="relative w-full h-full overflow-hidden">

        <div className="relative w-full h-full flex items-end justify-center translate-x-[460px]">
          {[3, 2, 1, 0].map((slotIndex) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="absolute top-[105%] left-0 w-full transition-all duration-700 text-left"
                  style={{
                    opacity: isActive ? 1 : 0.8,
                    transformOrigin: 'left top'
                  }}
                >
                  <p className="text-zinc-500 text-[11px] font-bold uppercase tracking-[0.4em] mb-2">
                    {campaign.date}
                  </p>
                  <h3 className={`text-white font-bold tracking-tighter leading-none transition-all duration-500 text-xl`}>
                    {campaign.title}
                  </h3>
                </div>
              </div>
