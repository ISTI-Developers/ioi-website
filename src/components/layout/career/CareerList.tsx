...
export default function CareerList() {
 ...
 const careers: Career[] = data ?? [];

const groupedByDepartment = careers
  .filter((c) => c.is_active)
  .reduce((acc: Record<string, Career[]>, career: Career) => {
  ...
  if (isLoading) {
    return (
      <div className="text-white text-center py-20">Loading careers...</div>
    );
  }
      ...
      {Object.entries(groupedByDepartment).map(([department, jobs], index) => (
        <div key={department} className="mb-20">
          ...
          {jobs.map((job, index) => (
            <section
              key={job.career_id}
              className="text-white flex flex-col py-16 border-b border-white/50"
            >
